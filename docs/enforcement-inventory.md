> **RECOVERY-1 status: PARTIAL inventory, not an adopted contract.** This file is
> the RECOVERY-1 normative inventory (issue #236). It covers the load-bearing
> rules; the remaining normative documents named in "Pass 1" below are not yet
> folded in. Its dispositions are the operator's proposals. The contract it
> points to is **not adopted** — adoption turns on OWNER-ONLY decisions
> (independence model, operator merge authority, baseline sign-off) recorded in
> #236 and unresolved here.
...
| **An acceptance bundle written by the operator is "Behauptung, nicht Feststellung"** until the second review of §12 exists | `docs/execution-core-adr.md:329` | Nothing marks it. Every oracle on 2026-08-10 was written by the operator, so every acceptance that day carries that status by the ADR's own terms and none of them says so. The ADR names this the `wo-98-3` class and insists it "muss hier stehen, nicht implizit bleiben". |
| **A standalone `no_network` preservation probe is mandatory before any change to the sandbox profile** | `docs/execution-core-adr.md:252` | The probe does not exist; `no_network` and `fs_confinement` still share one preservation test. #232 changed the profile without it. The ADR states the exact failure this guards: a profile regression that breaks legitimate work without breaking the `fs_confinement` load passes green today. |
| **`operator-v1.2-runtime-state-adr.md` is cited as the binding contract while its own status is PROPOSED** | `CLAUDE.md:141` cites it as "the full contract"; the ADR header reads `Status: PROPOSED` | `check_amendment_log` only fires on `Status: Accepted`, so a PROPOSED document being treated as binding is invisible to it. Either the status is wrong or the citation is. |
| A backend may only claim capabilities its canaries prove | `docs/execution-core-adr.md:206` | `core/launcher.py` imports `backends.macos_seatbelt` by name at two sites; `scripts/review.sh` builds its own profile at a third. No capability negotiation exists. Issue #233. **The weight of this is higher than first recorded**: the S1 backend is not a paper design. Measured 2026-08-10 — slot users `_jcode_w01`–`_jcode_w08` exist at uid 611–618, sudoers carries NOPASSWD entries for `worker_exec.sh`, and `/usr/local/var/jcode-runs` is root-owned and unreadable to the operator, which is the hardening `docs/s1-setup.md` specifies. A provisioned, uid-isolated, canary-proven sandbox was sitting on the machine while #232 built a weaker one inline. An earlier revision of this file inferred from an `ls` that the execution core was not in operation; the directory is root-owned and the inference was wrong. |
